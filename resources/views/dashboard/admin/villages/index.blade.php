            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex justify-end gap-2">
                        <button onclick='openPosyanduModal(@json($village))' class="text-green-600 hover:text-green-900 text-sm bg-green-50 px-3 py-1 rounded-md border border-green-200">
                            Kelola Posyandu
                        </button>
                        <button onclick="openEditModal({{ $village->id }}, '{{ $village->name }}')" class="text-indigo-600 hover:text-indigo-900 border border-indigo-200 px-3 py-1 rounded-md hover:bg-indigo-50">Edit</button>
                        <form action="{{ route('admin.villages.destroy', $village->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?');" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-600 hover:text-red-900 border border-red-200 px-3 py-1 rounded-md hover:bg-red-50">Hapus</button>
                        </form>
                    </div>
                </td>
            </tr>
...
<div id="posyanduModal" class="hidden fixed inset-0 z-50 overflow-y-auto bg-gray-900 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full p-6 mx-4">
        <div class="flex justify-between items-center mb-6">
            ...
            <button onclick="document.getElementById('posyanduModal').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">
        ...
        <div class="bg-gray-50 p-4 rounded-lg mb-6 border border-gray-100">
            ...
            <form action="{{ route('admin.posyandus.store') }}" method="POST" class="flex gap-2">
                ...
                <input type="text" name="name" placeholder="Nama Posyandu" required class="flex-1 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                <input type="text" name="address" placeholder="Alamat (Opsional)" class="flex-1 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 whitespace-nowrap">
                    + Tambah
                </button>
            </form>
        </div>
