        ...
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                    <button x-show="selectedItems.length > 0" @click="confirmBulkDelete()"
                    ...
                    <a href="{{ route('admin.users.export') }}"
                        class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition shadow-sm flex items-center">
                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <button @click="importModalOpen = true"
                        class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition shadow-sm flex items-center">
                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <form action="{{ route('admin.users') }}" method="GET" class="flex gap-2 max-w-md">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.users') }}"
                            class="px-4 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-200 transition flex items-center">
                            Reset
                        </a>
                    @endif
                </form>
        ...
        <div x-show="importModalOpen" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <form action="{{ route('admin.users.import') }}" method="POST" enctype="multipart/form-data" @submit="isImporting = true">
                        @csrf
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            ...
                            <div class="bg-yellow-50 border-l-4 border-yellow-400 p-3 rounded">
                                <p class="text-sm text-yellow-700">
                                    <strong>Tips:</strong> Download template terlebih dahulu untuk memastikan format data
                                    yang benar.
                                </p>
                    ...
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col"
                                ...
                                Anak</th>
                            <th scope="col"
                                class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Orang
                                Tua</th>
                            <th scope="col"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($users as $user)
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    <input type="checkbox" value="{{ $user->id }}" x-model="selectedItems"
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-xs text-gray-500">
                                        {{ $user->patient->gender == 'male' ? 'Laki-laki' : 'Perempuan' }}</div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                                    @if($isCompleted)
                                        <a href="{{ route('admin.certificate', urlencode($user->patient->certificate_number)) }}"
                                            target="_blank"
                                            class="inline-flex items-center px-3 py-1 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
                                            Unduh Sertifikat
                                        </a>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                    <a href="{{ route('admin.users.edit', $user->id) }}"
                                        class="inline-flex items-center px-3 py-1 bg-yellow-500 text-white rounded text-xs hover:bg-yellow-600 transition mr-2">
                                    ...
                                    <button @click='openDetailModal(JSON.parse($el.dataset.user))'
                                        data-user="{{ json_encode($user) }}"
                                        class="inline-flex items-center px-3 py-1 bg-cyan-600 text-white rounded text-xs hover:bg-cyan-700 transition mr-2">
                                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST"
                                        class="inline-block"
                                        onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini? Data akan diarsip (Soft Delete).');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="inline-flex items-center px-3 py-1 bg-red-600 text-white rounded text-xs hover:bg-red-700 transition">
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16">
                                                </path>
                            ...
                            <div class="md:col-span-1 bg-gray-50 p-4 rounded-lg">
                                ...
                                <div class="space-y-4">
                                    <div class="text-center mb-4">
                                        ...
                                        <p class="text-sm text-gray-500" x-text="detail.patient?.nik || 'NIK Belum diisi'">
                                        </p>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm font-medium text-gray-900"
                                            x-text="formatDate(detail.patient?.date_birth)"></p>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-600"
                                            x-text="detail.patient?.village?.name ? 'Dusun ' + detail.patient.village.name : ''">
                                        </p>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <template
                                        ...
                                        <div class="space-y-4">
                                            <template x-for="vp in detail.patient.vaccine_patients" :key="vp.id">
                                                <div
                                                    class="flex items-start p-3 bg-white border border-gray-100 rounded-lg shadow-sm hover:shadow-md transition">
                                                    ...
                                                    <div class="ml-4 flex-1">
                                                        ...
                                                        <div class="mt-1 text-sm text-gray-500">
                                                            <span
                                                                x-text="vp.status === 'selesai' ? 'Divaksin pada: ' : 'Jadwal: '"></span>
                                                            <span class="font-medium"
                                                                x-text="formatDate(vp.date || vp.start_date)"></span>
                                                        </div>
                                                        <template x-if="vp.village">
                                                            <p class="text-xs text-gray-400 mt-1">Lokasi: Dusun <span
                                                                    x-text="vp.village.name"></span></p>
                                                        </template>
                                                    </div>
                                                </div>
                                            </template>
