    ...
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="mother_name" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" required value="{{ old('mother_name', $user->patient->mother_name ?? '') }}">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Lahir Anak</label>
                <input type="date" name="date_birth" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" required value="{{ old('date_birth', $user->patient ? $user->patient->date_birth->format('Y-m-d') : '') }}">
            ...
            <div>
                ...
                <select name="gender" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" required>
                    <option value="male" {{ (old('gender', $user->patient->gender ?? '') == 'male') ? 'selected' : '' }}>Laki-laki</option>
            ...
            <div>
                ...
                <select name="village_id" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" required>
                    ...
                    @foreach($villages as $village)
                        <option value="{{ $village->id }}" {{ (old('village_id', $user->patient->village_id ?? '') == $village->id) ? 'selected' : '' }}>{{ $village->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="address" rows="2" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" required>{{ old('address', $user->patient->address ?? '') }}</textarea>
            </div>
