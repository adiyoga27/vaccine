...
@section('content')
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-500 mt-1">Buat jadwal rutin untuk setiap Dusun.</p>
        </div>
                    ...
                    @foreach($schedules as $schedule)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500">
                                <div class="flex flex-wrap gap-1">
                                    @foreach($schedule->vaccines as $vac)
                                        <span
                                            class="px-2 py-0.5 rounded text-xs bg-gray-100 text-gray-600 border border-gray-200">{{ $vac->name }}</span>
                                    @endforeach
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <span
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">Akan
                                        Datang</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    <button data-vaccines="{{ $schedule->vaccines->pluck('id') }}"
    ...
    <div id="createModal"
        ...
        <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <form action="{{ route('admin.schedules.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Pilih Dusun</label>
                        ...
                        <div class="space-y-2 border rounded-lg p-3 max-h-48 overflow-y-auto bg-gray-50">
                            @foreach($vaccines as $vaccine)
                                <div class="flex items-center">
                                    <input id="create_vaccine_{{ $vaccine->id }}" name="vaccine_ids[]"
                                        value="{{ $vaccine->id }}" type="checkbox"
                                        class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                                    <label for="create_vaccine_{{ $vaccine->id }}" class="ml-2 block text-sm text-gray-900">
                                        {{ $vaccine->name }} <span class="text-xs text-gray-500">(Min:
                                            {{ $vaccine->minimum_age }} Bln)</span>
                                    </label>
                                </div>
                            @endforeach
                        </div>
        ...
        <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <form id="editForm" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Pilih Dusun</label>
                        <select id="edit_village_id" name="village_id" required
                            class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @foreach($villages as $village)
                                <option value="{{ $village->id }}">{{ $village->name }}</option>
                            @endforeach
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Kegiatan</label>
                        <input id="edit_scheduled_at" type="date" name="scheduled_at" required
                            class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Vaksin Tersedia</label>
                        <div class="space-y-2 border rounded-lg p-3 max-h-48 overflow-y-auto bg-gray-50">
                            @foreach($vaccines as $vaccine)
                                <div class="flex items-center">
                                    <input id="edit_vaccine_{{ $vaccine->id }}" name="vaccine_ids[]" value="{{ $vaccine->id }}"
                                        type="checkbox"
                                        class="edit-vaccine-checkbox h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
