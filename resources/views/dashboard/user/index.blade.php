            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-bold text-xl"><span class="text-amber-500">TANDU</span> <span
                                class="text-emerald-600">GEMAS</span></span>
                    </div>
                    <div class="flex items-center gap-4">
                        @auth
                            <span class="text-gray-700 text-sm hidden sm:block">Halo, <span
                                    class="font-semibold">{{ Auth::user()->name }}</span></span>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                    class="bg-gray-100 hover:bg-gray-200 text-gray-600 px-4 py-2 rounded-lg text-sm font-medium transition">Keluar</button>
                            </form>
                        @else
                            <span class="text-gray-700 text-sm hidden sm:block">Halo, <span
                                    class="font-semibold">{{ $patient->name ?? 'Tamu' }}</span></span>
                            <a href="{{ url('/') }}"
                                class="bg-gray-100 hover:bg-gray-200 text-gray-600 px-4 py-2 rounded-lg text-sm font-medium transition">Beranda</a>
                        @endauth
                    </div>
                </div>
            </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                        ...
                        @if($allVaccinesCompleted)
                            <a href="{{ route('user.certificate') }}"
                                class="mt-6 w-full bg-emerald-600 hover:bg-emerald-700 text-white py-2 px-4 rounded-xl font-medium shadow-sm transition block text-center">
                                Download Sertifikat
                            </a>
                        @else
                            <div
                                class="mt-6 w-full bg-blue-50 border border-blue-200 text-blue-800 py-3 px-4 rounded-xl text-sm text-center">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                        ...
                        <div class="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                            @foreach($vaccineSchedules as $item)
                                <div
                                    class="flex items-center justify-between p-3 rounded-lg border {{ $item->status == 'selesai' ? 'border-emerald-100 bg-emerald-50' : ($item->status == 'bisa_diajukan' ? 'border-amber-100 bg-amber-50' : ($item->status == 'terlewat' ? 'border-red-100 bg-red-50' : 'border-gray-100 bg-white')) }}">
                                    <div>
                                        <p class="text-sm font-semibold text-gray-900">{{ $item->vaccine->name }}</p>
                                        <p class="text-xs text-gray-500 mt-0.5">Jadwal:
                                            {{ $item->start_date ? $item->start_date->format('d M Y') : 'N/A' }}</p>
                                        @if($item->end_date)
                                    ...
                                    <div>
                                        @if($item->status == 'selesai')
                                            <span
                                                class="px-2 py-1 bg-green-100 text-green-700 text-xs rounded-full font-bold">Selesai</span>
                                        @elseif($item->status == 'bisa_diajukan')
                                            <span
                                                class="px-2 py-1 bg-amber-100 text-amber-700 text-xs rounded-full font-bold animate-pulse">Saatnya
                                                Vaksin</span>
                                        @elseif($item->status == 'terlewat')
                                            <span
                                                class="px-2 py-1 bg-red-100 text-red-700 text-xs rounded-full font-bold">Terlewat</span>
                                        @else
                                            <span class="px-2 py-1 bg-gray-100 text-gray-500 text-xs rounded-full">Nanti
                                                ({{ $item->min_age }} Bln)</span>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
