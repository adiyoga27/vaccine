...
<div class="flex justify-between items-center mb-8">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Data Master Jenis Vaksin</h1>
        <p class="text-gray-500 mt-1">Kelola daftar vaksin dan persyaratan usia secara terpusat.</p>
    </div>
        ...
        <div>
            <div class="flex justify-between items-start mb-4">
                ...
                <div class="flex gap-2">
                    <button onclick="openEditModal({{ $vaccine->id }}, '{{ $vaccine->name }}', {{ $vaccine->minimum_age }}, {{ $vaccine->duration_days ?? 7 }}, {{ $vaccine->is_required ? 'true' : 'false' }})" class="text-gray-400 hover:text-indigo-600 transition">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                    </button>
                    <form action="{{ route('superadmin.vaccines.destroy', $vaccine->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?');">
                        @csrf
                        @method('DELETE')
...
<div id="createModal" class="hidden fixed inset-0 z-50 overflow-y-auto bg-gray-900 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold text-gray-900">Tambah Vaksin Baru</h3>
        ...
        <form action="{{ route('superadmin.vaccines.store') }}" method="POST">
            @csrf
            <div class="space-y-4">
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="minimum_age" value="0" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Durasi (Hari)</label>
                    <input type="number" name="duration_days" value="7" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    <p class="text-xs text-gray-500 mt-1">Default: 7 Hari (1 Minggu)</p>
                </div>
                <div class="flex items-center">
                    <input type="checkbox" name="is_required" id="create_is_required" value="1" checked class="w-4 h-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500">
...
<div id="editModal" class="hidden fixed inset-0 z-50 overflow-y-auto bg-gray-900 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold text-gray-900">Edit Vaksin</h3>
        ...
        <form id="editForm" method="POST">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Usia Minimum (Bulan)</label>
                    <input type="number" name="minimum_age" id="edit_minimum_age" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Durasi (Hari)</label>
                    <input type="number" name="duration_days" id="edit_duration_days" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                </div>
                <div class="flex items-center">
                    <input type="checkbox" name="is_required" id="edit_is_required" value="1" class="w-4 h-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500">
...
<script>
    function openEditModal(id, name, age, duration, isRequired) {
        document.getElementById('editForm').action = '/superadmin/vaccines/' + id;
